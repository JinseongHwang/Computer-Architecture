...
1.0GHz: 1 * 10^9 사이클: 1초 동안 회전 횟수 == 클록 속도   
클록 사이클 시간 = 1 / 10^9 (1ns): 1바퀴 회전하는데 걸리는 시간(클록 속도와 역수 관계)    
...
`CPU실행시간 / 프로그램`     
`= (명령어 개수 / 프로그램) * (사이클 개수 / 명령어) * (시간 / 사이클)`    
    
CPU 마다, 명령어 마다 사용되는 사이클 개수가 제각각이므로 평균 CPI값으로 계산한다.    
평균CPI란?(Average Cycles per Instruction): 각 명령어들의 평균 사이클 수    
    
(시간 / 사이클): 한 클록 사이클에 소요되는 시간. 즉 (클록) 사이클 시간을 의미한다.    
(사이클 / 시간): 단위시간동안 회전 횟수. 클록률 또는 클록 속도를 의미한다.    

위 식에서 분모에 전부 1을 넣어주면 아래의 식으로 정리할 수 있다.   
...
**교재 p.74 예제 2-2>**    
2-3에서 평균 CPI를 구해야 하는데, 포함되어 있는 같은 문제이기에 생략한다.   

**교재 p.74 예제 2-3>**    
|명령어의 종류|M1 사용빈도|M1 CPI|M2 사용빈도|M2 CPI|
|:---:|:---:|:---:|:---:|:---:|
|산술.논리|50%|1|40%|1|
|적재.저장|30%|2|30%|1|
|분기|20%|3|30%|4|
...
우리가 알아야 하는 정보는 프로그램 크기(명령어 개수), 평균 CPI, 클록 속도만 알면 된다.   
명령어 개수는 주어진 정보가 두 컴퓨터에서 동일하다라고만 주어졌으므로 n으로 부여한다.   
평균 CPI는 구했으므로, 클록 속도를 구해보자.   

M1 클록 속도 = 3GHz = 3 * 10^9    
M2 클록 속도 = 4GHz = 4 * 10^9    

M1의 실행시간 = (n * 1.7) / (3 * 10^9) = sM1    
M2의 실행시간 = (n * 1.9) / (4 * 10^9) = sM2    

s = sM1 / sM2 = 약 1.19   

∴ 따라서 M1보다 M2가 약 1.19배 빠르다.
